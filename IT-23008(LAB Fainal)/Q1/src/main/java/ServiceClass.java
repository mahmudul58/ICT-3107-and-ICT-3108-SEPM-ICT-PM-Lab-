...
public class ServiceClass extends DBConnection {
    ...
    // INSERT: Adds a new department with student count
    ...
    // VIEW: Retrieves all departments
    public List<String> viewDB() {
        ...
        try {
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
                result.add("Department: " + rs.getString("DEPT_NAME") + ", Students: " + rs.getInt("NUM_STUDENTS"));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        } finally {
    ...
    // UPDATE: Updates student count for a specific department
